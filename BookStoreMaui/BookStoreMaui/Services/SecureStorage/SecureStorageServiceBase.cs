...
public class SecureStorageServiceBase
{
    ...
    protected async Task<bool> GetBoolean(string key, bool defaultValue) => Convert.ToBoolean(await GetAsync(key, defaultValue.ToString()));
    protected async Task SaveDouble(string key, double value) => await SetAsync(key, value.ToString(CultureInfo.InvariantCulture));
    protected async Task<double> GetDouble(string key, double defaultValue) => Convert.ToDouble(await GetAsync(key, defaultValue.ToString(CultureInfo.InvariantCulture)));
    protected async Task SaveInteger(string key, int value) => await SetAsync(key, value.ToString());
    protected async Task<int> GetInteger(string key, int defaultValue) => Convert.ToInt32(await GetAsync(key, defaultValue.ToString()));
    protected async Task SaveLong(string key, long value) => await SetAsync(key, value.ToString());
    private async Task<long> GetLong(string key, long defaultValue) => Convert.ToInt64(await GetAsync(key, defaultValue.ToString()));
    protected async Task SaveDateTime(string key, DateTime value) => await SetAsync(key, value.ToString(CultureInfo.InvariantCulture));
    protected async Task<DateTime> GetDateTime(string key, DateTime defaultValue) => Convert.ToDateTime(await GetAsync(key, defaultValue.ToString(CultureInfo.InvariantCulture)));
